...
class Classroom():
    ...
    def compute_mean_score(self):
        """Calcula a media das pontuacoes de todos os estudantes da sala.
        """
        sum_score = []
        for index, student in self.students.items():
            sum_score += [student.score / len([i for i in student.original_progression if i is not None])]
        self.mean_score = np.mean(sum_score)