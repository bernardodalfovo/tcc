...
class Classroom():
    def __init__(self, attendance_report_doc: any, grade_report_doc: any, activity_report_doc: any) -> None:
        self.students = {}
        # self.attendance_weights = []
        # self.mean_attendance_score = 0.0

        self.attendance_report = attendance_report_doc
        self.grade_report = grade_report_doc
        self.activity_report = activity_report_doc

        for index, name in attendance_report_doc["Nome"].items():
            self.students[index] = Student(name=name, index=index)
    
    # def measure_weights_attendance(self):
    ...
    #                 weight_date += [self.mean_attendance_score]
    #         weight_array += [weight_date]
    #     weight_array_new = []
    #     for date in weight_array:
    #         weight_array_new += [np.mean(date)]
    #     self.attendance_weights = np.array(weight_array_new)
    ...
    # def compute_mean_attendance_score(self):
    #     """Calcula a media das pontuacoes relacionadas a presenca da sala.
    #     """
    #     sum_score = []
    #     for index, student in self.students.items():
    #         sum_score += [student.attendance_score / len([i for i in student.original_progression if i is not None])]
    #     self.mean_attendance_score = np.mean(sum_score)